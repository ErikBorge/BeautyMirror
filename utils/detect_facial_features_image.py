...
def getForeheadArea(image, gray, rects):
	if rects:
...
def getEyes(image, gray, rects):
	if rects:
		for (i, rect) in enumerate(rects):
			...
			refvector=[1,0]
			anglerad=angle_between(LRvector,refvector)
...
def getFacialFeatures(image):
        ...
        #skin color
        roiforehead, img = getForeheadArea(image, gray, rects)
        skincolor = getAverageColor(roiforehead)
...
getFacialFeatures(cv2.imread('erik.jpeg'))
